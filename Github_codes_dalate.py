...
# Oxirgi statusni saqlash
last_status = None

IS_ACTIVE = True

async def is_online():
    """Foydalanuvchi online yoki offline ekanini tekshiradi"""
...
async def send_auto_offline_message(event):
    ...
    # Agar hozir online bo'lsa, hech narsa qilmaymiz
    if online:
        print(f"[{event.sender_id}] dan xabar keldi, lekin siz online bo‘lgansiz, javob berilmadi.")
        return

    # Offline bo'lsa avtomatik javob beramiz
    try:
        await event.respond(away_message, parse_mode="html")
